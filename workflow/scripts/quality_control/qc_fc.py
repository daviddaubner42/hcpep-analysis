...
fds_df_rot = motion[["subid", "ses", "dir", "mean_fd_rot"]]
...
for ses in [1, 2]:
    for dir in ['AP', 'PA']:
        ...
        for subid in subid_order:
            fds_trans.append(fds_df_trans[(fds_df_trans['subid'] == int(subid)) & (fds_df_trans['ses'] == ses) & (fds_df_trans['dir'] == dir)].mean_fd_trans.item())
            fds_rot.append(fds_df_rot[(fds_df_rot['subid'] == int(subid)) & (fds_df_rot['ses'] == ses) & (fds_df_rot['dir'] == dir)].mean_fd_rot.item())
        ...
        fig.savefig(f"{args.output_dir}/figures/ses-{ses}_task-rest_dir-{dir}_desc-qcFcCorrelationHistograms_figure.png", bbox_inches="tight")
        ...
        res = pearsonr(
            ...
            corrs_trans[np.triu_indices_from(corrs_trans, k=1)]
        )
        ...
        res = pearsonr(
            dists[np.triu_indices_from(dists, k=1)],
            corrs_rot[np.triu_indices_from(corrs_rot, k=1)]
        )
        ...
        ax[0].scatter(dists[np.triu_indices_from(dists, k=1)], 
            corrs_trans[np.triu_indices_from(corrs_trans, k=1)], s=1)
        ...
        ax[1].scatter(dists[np.triu_indices_from(dists, k=1)], 
            corrs_rot[np.triu_indices_from(corrs_rot, k=1)], s=1)
        ...
        fig.savefig(f"{output_dir}/figures/ses-{ses}_task-rest_dir-{dir}_desc-qcFcDistanceCorrelation_figure.png", bbox_inches="tight")
